...
class NoticeCreateRequest(BaseModel):
    ...
    attachment_name: Optional[str] = Field(None, description="단일 첨부파일명(레거시 하위호환)")
    attachment_url: Optional[str] = Field(None, description="단일 첨부파일 URL(레거시 하위호환)")
    attachments: Optional[List[dict]] = Field(default_factory=list, description="복수 첨부파일 리스트")
...
def ensure_board_tables(db: Session):
    try:
        ...
        db.execute(text("ALTER TABLE system_notices ADD COLUMN IF NOT EXISTS attachments TEXT;"))
        ...
        db.execute(text("ALTER TABLE community_posts ADD COLUMN IF NOT EXISTS attachments TEXT;"))
...
def parse_attachments_json(att_text: str, name_fallback: str, url_fallback: str) -> List[dict]:
    result = []
    if att_text:
        try:
            parsed = json.loads(att_text)
            if isinstance(parsed, list):
                result = parsed
        except Exception:
            pass
    if not result and name_fallback and url_fallback:
        result = [{"attachment_name": name_fallback, "attachment_url": url_fallback}]
    return result
...
async def upload_board_attachments(files: List[UploadFile] = File(...)):
    """복수(멀티) 첨부파일 동시 업로드 API"""
    try:
        uploaded_list = []
        for file in files:
            clean_filename = os.path.basename(file.filename)
        ...
        return {
            "status": "success",
            "attachments": uploaded_list,
            "count": len(uploaded_list),
            "message": f"총 {len(uploaded_list)}개 첨부파일이 성공적으로 업로드되었습니다."
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"멀티 첨부파일 저장 처리 실패: {str(e)}")
...
async def get_notices(db: Session = Depends(get_db)):
    ...
    try:
        rows = db.execute(text("SELECT id, title, content, is_pinned, author, created_at, attachment_name, attachment_url, attachments FROM system_notices ORDER BY is_pinned DESC, id DESC")).fetchall()
    except Exception:
        db.rollback()
        rows = []
    ...
    result = []
    for r in rows:
        att_name = r[6] if len(r) > 6 else None
        att_url = r[7] if len(r) > 7 else None
        att_text = r[8] if len(r) > 8 else None
        att_list = parse_attachments_json(att_text, att_name, att_url)
        ...
        result.append({
            ...
            "attachment_name": att_list[0]["attachment_name"] if att_list else None,
            "attachment_url": att_list[0]["attachment_url"] if att_list else None,
...
async def create_notice(req: NoticeCreateRequest, db: Session = Depends(get_db)):
    ...
    # Process attachments
    att_list = req.attachments or []
    if not att_list and req.attachment_name and req.attachment_url:
        att_list = [{"attachment_name": req.attachment_name, "attachment_url": req.attachment_url}]
    
    single_name = att_list[0]["attachment_name"] if att_list else None
    single_url = att_list[0]["attachment_url"] if att_list else None
    att_json = json.dumps(att_list, ensure_ascii=False) if att_list else None

    db.execute(text("""
        INSERT INTO system_notices (title, content, is_pinned, author, attachment_name, attachment_url, attachments)
        VALUES (:title, :content, :is_pinned, '시스템 최고 관리자', :attachment_name, :attachment_url, :attachments)
    """), {
        "title": req.title, 
        "content": req.content, 
        "is_pinned": req.is_pinned,
        "attachment_name": single_name,
        "attachment_url": single_url,
        "attachments": att_json
    })
    db.commit()
    try:
        from app.routers.spatial import save_pipeline_log
        save_pipeline_log(db, 'BOARD', '[NOTICE_CREATE]', {'title': req.title, 'is_pinned': req.is_pinned, 'attachments_count': len(att_list)})
...
async def update_notice(notice_id: int, req: NoticeUpdateRequest, db: Session = Depends(get_db)):
    ...
    result = db.execute(text("""
        ...
        SET title = :title, content = :content, is_pinned = :is_pinned, attachment_name = :attachment_name, attachment_url = :attachment_url, attachments = :attachments 
        WHERE id = :id
    """), {
        "title": req.title, 
        "content": req.content, 
...
async def get_community_posts(db: Session = Depends(get_db)):
    ensure_board_tables(db)
    try:
        rows = db.execute(text("SELECT id, title, content, author_name, department, views_count, created_at, attachment_name, attachment_url, attachments FROM community_posts ORDER BY id DESC")).fetchall()
    except Exception:
        db.rollback()
        rows = []
    ...
    result = []
    for r in rows:
        att_name = r[7] if len(r) > 7 else None
        att_url = r[8] if len(r) > 8 else None
        att_text = r[9] if len(r) > 9 else None
        att_list = parse_attachments_json(att_text, att_name, att_url)
...
async def create_community_post(req: PostCreateRequest, db: Session = Depends(get_db)):
    ...
    db.execute(text("""
        INSERT INTO community_posts (title, content, author_name, department, attachment_name, attachment_url, attachments)
        VALUES (:title, :content, :author_name, :department, :attachment_name, :attachment_url, :attachments)
    """), {
        "title": req.title,
        "content": req.content,
    ...
    try:
        from app.routers.spatial import save_pipeline_log
        save_pipeline_log(db, 'BOARD', '[COMMUNITY_POST_CREATE]', {'title': req.title, 'author': req.author_name, 'department': req.department, 'attachments_count': len(att_list)})
    except Exception as log_err:
