...
from app.services.llm_audit_service import analyze_audit_document_via_llm
...
def get_rag_matched_regulations(db: Session, query_str: str, facility_type: Optional[str] = None, limit: int = 3) -> list:
    """[v4.9.48] 100% 하드코딩 배제 순수 동적 RAG 조례 매칭 엔진 (OpenAI 코사인 유사도 + DB 텍스트 검색)"""
    matched_regulations = []
    
    # 1. OpenAI Vector Embedding 기반 pgvector 코사인 유사도 연산 (영벡터 제외 및 임계값 0.20 이상)
    client = get_openai_client()
    if client and query_str and query_str.strip():
        try:
            embed_res = client.embeddings.create(
                model="text-embedding-3-small",
                input=query_str[:1000]
            )
    ...
    try:
        ...
        query = text("""
            SELECT 
                ...
                vp.actual_scenario, 
                COALESCE(TO_CHAR(vp.created_at, 'YYYY-MM-DD HH24:MI'), TO_CHAR(dh.created_at, 'YYYY-MM-DD HH24:MI'), '2026-08-08 00:00') AS date_str, 
                vp.match_score, 
                vp.audit_opinion,
