# OmniSite 지능형 다목적 SDSS 종합 연구노트 & 주니어 개발자 연대기 교본 (v1.0.0-Production)

본 문서는 B2G 스마트시티 다기준 의사결정 지원 시스템(SDSS) **OmniSite**의 탄생(0.1 기획)부터 최종 프로덕션 상용화 버전(**v1.0.0-Production**)에 도달하기까지의 전체 아키텍처 진화 과정, 데이터베이스 스크립트 설계, GIS 공간 연산의 기하학적 난제 극복, 그리고 현업 인프라 이식 시 필요한 환경 튜닝을 백과사전식으로 편찬한 **주니어 엔지니어링 가이드 및 연구노트**입니다.
