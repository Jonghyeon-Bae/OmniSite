...
const STEP_GUIDE_DATA = {
  1: {
    ...
    items: [
      {
        head: '📋 임의 공공 공간 데이터셋 (CSV) 자유 업로드',
        body: '특정 한글/영문 컬럼명 규격 제한 없이, 분석하고자 하는 스마트 인프라 후보지/현황 CSV 데이터를 자유롭게 업로드'
      },
      {
        head: '🧠 LLM 기반 유연한 AI 감리 (Zero-Schema Dependency)',
        body: 'DB 직접 저장 없이 캐싱 후 OpenAI LLM이 CSV 텍스트와 의미(Context)를 자동 해독하여 인프라 수립 목적과 지표를 알아서 추론'
      },
      {
        head: '📜 RAG 조례 PDF 교차 연동',
        body: '지자체 자치법규 조례 PDF를 추가 적재하면 [조례명 > 제N조] 파싱 기반의 AI 교차 준공 감리 준비가 완비됩니다.'
      }
    ]
  },
  2: {
