...
import bcrypt
try:
    from app.scripts.optimize_spatial_relations import optimize_spatial_relations
except (ImportError, ModuleNotFoundError):
    try:
        from backend.app.scripts.optimize_spatial_relations import optimize_spatial_relations
    except (ImportError, ModuleNotFoundError):
        def optimize_spatial_relations(*args, **kwargs):
            pass
...
def resolve_path(key, default_fallback):
    ...
    # 1. Datasets 후보 경로 다중 탐색 (루트, backend, /workspace 컨테이너 환경)
    candidate_datasets_bases = [
    ...
    for datasets_base in candidate_datasets_bases:
        ...
        try:
            ...
            try:
                ...
                for reg in default_regulations:
                    conn.execute(text("""
                        INSERT INTO district_regulations (district_id, regulation_title, clause_number, content, category, version_tag)
                        VALUES (1, :title, :clause, :content, :category, 'v1.0')
