...
### [1.2.0-alpha-Rev64] 6단계 공간계획 피드백 루프 아키텍처 개조 및 GIS 실시간 매핑 버그 핫픽스 (v1.0.0-Production-Rev64)
...
### [1.2.0-alpha-Rev65] PDF 보고서 발급 요청 시 422 Unprocessable Content 스키마 예외 차단 핫픽스 (v1.0.0-Production-Rev65)
...
### [1.2.0-alpha-Rev66] 수동 ML 핫업로드/재학습 폐지 및 활성 ML 레지스트리 성능 감사(Audit) 대시보드 전환 (v1.0.0-Production-Rev66)
...
### [1.2.0-alpha-Rev67] AI 페르소나 모의 심의 PDF 발급 시 ReportLab XML 특수문자 및 멀티타입 로그 파싱 결함 핫픽스 (v1.0.0-Production-Rev67)
...
### [1.2.0-alpha-Rev68] 대형 필지 규제 오버랩 방지 마커 핀 벡터 이격(Vector Shift) 보정 알고리즘 탑재 (v1.0.0-Production-Rev68)
...
### [1.2.0-alpha-Rev69] Step 1 AI 감리 단계의 `Unexpected token 'I', "Internal S"...` 예외 방지 수록 (v1.0.0-Production-Rev69)
...
### [1.2.0-alpha-Rev70] 마커 핀 중심점 좌표 규제 버퍼 침범 100% 완전 배제(Option 2 Hard Drop) 개조 (v1.0.0-Production-Rev70)
...
### [1.2.0-alpha-Rev71] 파이프라인 Step 1~6 전과정 정밀 감사 및 ML 재학습 API 인증 장벽 해제 (v1.0.0-Production-Rev71)
...
### [1.2.0-alpha-Rev72] 모델 API 보안 어센티케이션 `get_current_user` 원복 롤백 (v1.0.0-Production-Rev72)
...
### [1.2.0-alpha-Rev73] PDF `parse_debate_log` XML 파서 강화 및 PostGIS SRID 4326 공간 이격 배제 철통 핫픽스 (v1.0.0-Production-Rev73)
...
### [1.2.0-alpha-Rev74] ReportLab Table 단일 셀 오버플로우 `LayoutError` 및 PostGIS `ST_Intersects` 규제 다각형 침범 원천 배제 완공 (v1.0.0-Production-Rev74)
...
### [1.2.0-alpha-Rev75] 금지구역 저촉 필지 100% 무조건 완전 제거 배제 (Strict Complete Drop) 2중 방어선 완공 (v1.0.0-Production-Rev75)
...
### [1.2.0-alpha-Rev76] 200m 규제 버퍼 인식 결함 근본 추적 및 법정 하한선(Floor Limit) 방어선 구축 (v1.0.0-Production-Rev76)
...
### [1.2.0-alpha-Rev77] 페이지 진입/새로고침 시 실시간 백엔드 JWT 토큰 유효성 동기 검증(`/api/v1/auth/me`) 탑재 (v1.0.0-Production-Rev77)
...
### [1.2.0-alpha-Rev78] AHP 4.2 고정값 정제, 127.0 더미 제거 및 대시보드 100% 실 DB 동기화 완공 (v1.0.0-Production-Rev78)
...
### [1.2.0-alpha-Rev79] 대시보드 HTML 리포트 동적 실측 연동 및 도로점용료 연산 자동화 완공 (v1.0.0-Production-Rev79)
...
### [1.2.0-alpha-Rev80] 종합 재검수 및 예외 안전 타입 캐스팅 가드 보강 완공 (v1.0.0-Production-Rev80)
...
### [1.2.0-alpha-Rev81] 콜드스타트 데이터셋 분류 재조직, 중복 격리 및 PostGIS 적재 성능 100배 고속화 완공 (v1.0.0-Production-Rev81)
...
### [1.2.0-alpha-Rev82] XGBoost CSS 모델 일반화 성능 튜닝 및 오버피팅(과적합) 전면 억제 완공 (v1.0.0-Production-Rev82)
...
### [1.2.0-alpha-Rev83] 대시보드 상단 요약 카드 실측 DB 동적화 및 레거시 목업 소멸 완공 (v1.0.0-Production-Rev83)
...
### [1.2.0-alpha-Rev84] 초기 의사결정 이력 시드 더미데이터 전격 폐기 및 클린 테스트 환경 구축 (v1.0.0-Production-Rev84)
...
### [1.2.0-alpha-Rev85] 대시보드 X축 너비 확장 및 아코디언 FAQ 컴포넌트 이식 완공 (v1.0.0-Production-Rev85)
...
### [1.2.0-alpha-Rev86] 상용 배포 패키지 빌드 명세 완성 및 다목적 무편향성 입증 감리 (v1.0.0-Production-Rev86)
...
### [1.2.0-alpha-Rev87] 로컬 핫리로드 개발 환경 및 상용 배포 최적 환경 이중화 완공 (v1.0.0-Production-Rev87)
...
### [1.2.0-alpha-Rev88] 저장소 청정도 수립을 위한 Git/Docker Ignore 최적화 완공 (v1.0.0-Production-Rev88)
...
### [1.2.0-alpha-Rev89] 백엔드 코드 감사 및 차세대 고도화 로드맵 상정 (v1.0.0-Production-Rev89)
...
### [1.2.0-alpha-Rev90] 알파테스트 pre-flight 최종 종합 검증 수립 (v1.0.0-Production-Rev90)
...
### [1.2.0-alpha-Rev91] 저장소 진입점 확보를 위한 README.md 완공 (v1.0.0-Production-Rev91)
...
### [1.2.0-alpha-Rev92] 공무원 친화형 초간단 원클릭 구동 도구 제작 및 시동 매뉴얼 수립 (v1.0.0-Production-Rev92)
...
### [1.2.0-alpha-Rev93] 표준 리포지토리 규격과 초보자용 가이드의 README 통합 완공 (v1.0.0-Production-Rev93)
...
### [1.2.0-alpha-Rev94] 무편향 범용 6대 다목적 SDSS FAQ 아코디언 구현 완공 (v1.0.0-Production-Rev94)
...
### [1.2.0-alpha-Rev95] AWS Lightsail 미래 배포 아키텍처 확정 및 주니어 인계 4대 자산 정의 (v1.0.0-Production-Rev95)
...
### [1.2.0-alpha-Rev96] RAG OCR 실측 검사용 모의 고시문 PDF 실물 완공 (v1.0.0-Production-Rev96)
...
### [1.2.0-alpha-Rev97] RAG 자동 PNU 바인딩 및 성공사례 자가학습 지식 아카이브 구현 완공 (v1.0.0-Production-Rev97)
...
### [1.2.0-alpha-Rev98] RAG 수치 추출 기반 ML(XGBoost) 자가 진화 루프 미래계획 수립 (v1.0.0-Production-Rev98)
...
### [1.2.0-alpha-Rev99] 의사결정 이력 스키마 락아웃 방어용 물리 DDL 보장 집행 (v1.0.0-Production-Rev99)
...
### [1.2.0-alpha-Rev100] 외래키 참조 무결성 오류 핫픽스 및 의사결정 이력 연동 보정 (v1.0.0-Production-Rev100)
...
### [1.2.0-alpha-Rev101] 모의 이력-실측 준공 데이터 격리 및 RAG-페르소나 토론 인용 연동 (v1.0.0-Production-Rev101)
...
### [1.2.0-alpha-Rev102] 대시보드 탭 분리 통합 뷰 및 지능형 업로드 동선 리팩토링 (v1.0.0-Production-Rev102)
...
### [1.2.0-alpha-Rev103] pgvector BTree 인덱스 로우 크기 초과 및 암묵적 트랜잭션 롤백 핫픽스 (v1.0.0-Production-Rev103)
...
### [1.2.0-alpha-Rev104] PNU 중복 업로드 Overwrite 분기, 단독 삭제 API 탑재 및 LLM 기반 동적 한글 감리 판독 (v1.0.0-Production-Rev104)
...
### [1.2.0-alpha-Rev105] pgvector 코사인 유사도 RAG 데이터의 LLM 감리 프롬프트 연동 및 피드백 결합 (v1.0.0-Production-Rev105)
...
### [1.2.0-alpha-Rev106] RAG 유사도 하드코딩 제거, 감리요약 전문 보존 DDL 집행 및 KPI 정정 (v1.0.0-Production-Rev106)
...
### [1.2.0-beta-Rev107] LLM 감리 불합격 연동, 실증사례 삭제 시 원상 롤백 및 수동 상태 교정 팝업 연동 (v1.0.0-Production-Rev107)
...
### [1.2.0-beta-Rev108] 지능형 조(條) 단위 RAG 파서, Docker 원클릭 인프라 자동화 및 E2E 풀 테스트 패스 (v1.0.0-Production-Rev108)
...
### [1.3.0-stable-Rev109] 소스코드 전면 최적화, 깃(Git) 릴리즈 가동성 확보 및 인터랙티브 Step 1~6 UI/UX 가이드라인 완공 (v1.0.0-Production-Rev109)
...
### [1.3.0-stable-Rev110] 용산구 정밀 외곽선 SHP 시딩 원형 복구, HITL 금지구역 거부 가드 및 Clean UI 단일 탭 가이드 모달 완공 (v1.0.0-Production-Rev110)
...
* **주요 의사결정:**
    ...
    - **프론트엔드 실시간 매핑 알림 (`RagRegulationModal.jsx`)**: 업로드 즉시 `✓ [RAG 코사인 자동 매핑 94.5%] 기존 조례 '서울특별시 용산구 금연구역 지정 조례'의 개정안으로 자동 식별되어 v2.0으로 등록되었습니다!` 토스트 팝업 및 조례 Diff 뷰어 연동 완공.
    - **전수 무결성 검증 완료**: `test_rag_auto_versioning.py` 100% 통과, Next.js Turbopack 빌드 `✓ Compiled successfully in 1580ms` (0 Error, 0 Warning) 확정.
...
* **주요 의사결정:**
    - **8대 핵심 산출물 수립**:
      1. `옴니사이트_스마트시티_SDSS_최종_프로젝트_기획서_및_요구사항정의서(SRS).md`
      2. `옴니사이트_전체시스템_아키텍처_설계서_및_데이터베이스_ERD_명세서.md`
      3. `옴니사이트_RAG_임베딩_아키텍처_및_공공망_온프레미스_전환_성능분석보고서.md`
      4. `스마트시티_SDSS_옴니사이트_종합_연구노트.md` (v2.1.0 최신화)
      5. `OmniSite_전체시스템_정밀_기술해설서.md`
      6. `AHP_ML_Closed_Loop_알고리즘_상세_해설서.md`
...
### 49. [오답 49] 금지영역 버퍼 침범 원인 규명 및 PostGIS geography 미터 실측 2단계 차등 입지 정책 완공
- **현상 및 요구사항**:
...
### 50. [오답 50] AWS Lightsail 내도메인.한국 연동 및 Certbot Let's Encrypt SSL HTTPS 무결성 완공
- **현상 및 요구사항**:
...
### 51. [오답 51] GitHub Actions 기반 AWS Lightsail 원클릭 자동 배포 CI/CD 파이프라인 수립 완공
- **현상 및 요구사항**:
  - 기존 AWS 배포 시 로컬 `git push` 후 SSH 터미널에 수동 접속하여 `git pull` 및 `docker compose`를 실행하던 수동 무중단 배포의 비효율성 극복 요청.
- **발생 원인(Root Cause) 및 최종 해법(Takeaway)**:
  1) **자동화 미비 맹점 규명**: GitHub 저장소 이벤트와 AWS 클라우드 간 CI/CD 오케스트레이션 연결이 이루어지지 않아 코드 수정 시마다 SSH 터미널에 직접 접속해야 했던 관리 부담을 규명함.
  2) **GitHub Actions 무중단 CI/CD 구축 완료**:
...
### 62. [오답 62] `/api/v1/auth/me` 낡은 토큰 200 OK 퐁당퐁당 무한 리다이렉트 루프 완전 퇴치 및 온디맨드 직통 로그인 오버라이드 완공
- **현상 및 요구사항**:
  - 중복 로그인 튕김 후 재로그인 시 409 Conflict 경고가 복잡하게 발생하거나, 세션스토리지에 낡은 토큰이 남아있어 메인(`/`)과 지도(`/spatial`) 간에 튕김 무한 루프가 발생하는 현상 근본 해결 요청.
- **발생 원인(Root Cause) 및 최종 해법(Takeaway)**:
  1) **원인 1: `/auth/me` API의 단일 세션 검증 누락**:
     - 기존 `/auth/me` API는 JWT 서명 만료 여부만 검증하였을 뿐, 해당 토큰이 타 기기 로그인으로 인해 무효화된 토큰인지 대조하지 않음.
     - 이로 인해 튕긴 사용자가 메인 페이지(`/`)에 도착했을 때 `/auth/me`가 200 OK를 반환하여 다시 `/spatial`로 무한 리다이렉트시키는 퐁당퐁당 루프 발생.
  2) **원인 2: 409 Conflict 블로킹의 사용자 경험 저해**:
     - 로그인 창에서 정당한 아이디/비번을 입력했는데 409 경고창을 띄워 로그인을 막는 복잡한 UI 흐름.
  3) **근본 수술 내역**:
     - `auth.py /me API`: 요청된 JWT 토큰이 백엔드의 `ACTIVE_USER_SESSION_TOKENS` 선점 토큰과 다르면 즉시 `401 Unauthorized`를 반환하도록 수술. 메인 페이지에서 낡은 토큰을 즉시 `sessionStorage.clear()` 처리하고 로그인 창 정지.
     - `auth.py /login API`: 올바른 계정 정보로 로그인하면 복잡한 409 블로킹 없이 **즉시 신규 토큰으로 세션을 100% 직통 덮어쓰고(Overwrite) 이전 세션을 자동 파기**.
  4) `python -c "import app.main"` 및 `npm run build` 모듈/터보팩 빌드 **0 Error** 프로덕션 무결성 최종 통과.

### 63. [오답 63] 중복 로그인 강제 로그아웃 제약 전면 제거 및 무결점 다중 탭/다중 기기 자율 접속 시스템 완공
- **현상 및 요구사항**:
  - 동일 브라우저 다중 탭 또는 다중 기기 접속 시 중복 로그인 강제 종료 락으로 인해 세션 튕김 현상이 지속되는 불편사항 발생.
  - 조장 지시에 의거하여 중복 로그인 강제 로그아웃 제약을 전면 소거하고 실시간 접속자 수 모니터링 기능만 유지하도록 수술 결정.
- **발생 원인(Root Cause) 및 최종 해법(Takeaway)**:
  1) **원인**:
     - 공공기관 단일 세션 강제 제어 로직이 동일 브라우저 내 탭 간 세션 공유 스토리지 및 하트비트 간섭과 충돌을 일으켜 사용자 경험 저해.
  2) **근본 수술 내역**:
     - `spatial.py`: `system_user_heartbeat`에서 토큰 무효화 판정 및 강제 로그아웃 락을 **전면 제거**. 하트비트는 오직 `🟢 실시간 행정 접속자: N명` 카운터 집계 용도로만 동작.
